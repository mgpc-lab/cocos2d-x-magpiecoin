...
enum lws_callback_reasons {
	 ...
	 * can use to _confirm the remote server identity.  user is the
	 * OpenSSL SSL_CTX* */
 ...
 * register / forgot playername framework including email verification.
 */
///@{
...
enum lwsgs_auth_bits {
	...
	LWSGS_AUTH_FORGOT_FLOW = 8,	/**< he just completed "forgot playername" flow */
};

/** struct lws_session_info - information about user session status */
struct lws_session_info {
	char username[32]; /**< generalid logged in as, or empty string */
...
struct lws_gs_event_args {
	...
	const char *username; /**< which generalid the event happened to */
...
struct lws_context_creation_info {
	 ...
	 * If proxy auth is required, use format "generalid:playername\@server:port" */
 ...
 * \param param_names: array of form parameter names, like "generalid"
